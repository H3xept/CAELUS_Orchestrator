...
class ProcessManager():
    ...
    def reschedule_process(self, old_p):
        self.schedule_process(old_p.get_docker_image(), old_p.get_mission_file_path())
    ...
    def __get_active_processes(self):
        ...
        return { pid:Process.status_to_string(p.status) for pid, p in self.__active_ps.items() } if items != {} else None

    def __get_old_processes(self):
        items = self.__old_ps.items()
        return { pid:Process.status_to_string(p.status) for pid, p in self.__old_ps.items() } if items != {} else None
